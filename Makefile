...
APP_VER				:= 1024
APP_TITLE			:= FridgeMic
APP_DESCRIPTION		:= A 3DS mic streaming tool.
APP_AUTHOR			:= TehFridge
PRODUCT_CODE		:= FMIC3DS
UNIQUE_ID			:= 0x32730

BANNER_AUDIO		:= meta/banner.wav
BANNER_IMAGE		:= meta/banner.png
ICON				:= meta/icon.png
RSF_PATH			:= meta/mic.rsf
...
CFLAGS	:=	-g -Wall -DMODE_DEBUG -O0 -mword-relocations \
			-ffunction-sections \
			$(ARCH) 
...
LIBS	:=  -lcitro2d -lcitro3d -lctru -lm -lcwav -lncsnd -ljansson
...
# Recursively find .t3s files inside gfx/, keeping paths relative to that folder
GFXFILES := $(foreach dir,$(GRAPHICS),$(patsubst $(dir)/%,%,$(wildcard $(dir)/**/*.t3s)))
